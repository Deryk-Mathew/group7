...
function process_query($yql_query_url,$dbh){
	...
	try{ 
		...
		$query = "INSERT INTO `kgbmxvry_u285514251_wms`.`stocks` (`averageDailyVolume`, `change`,`daysLow`, `daysHigh`, `yearLow`, `yearHigh`, `marketCapitalization`, `lastTradePriceOnly`, `gbp_price`, `daysRange`, `name`, `symbol`, `volume`, `exchange_id`) VALUES (:averagedailyvol,:change,:dayslow,:dayshigh,:yearlow,
		:yearhigh,:marketcap,:lasttradeprice,:lasttradeprice*(1/(SELECT rate FROM `kgbmxvry_u285514251_wms`.`stock_exchanges`,`kgbmxvry_u285514251_wms`.`exchange_rates` WHERE `stock_exchanges`.`id` = 
(SELECT id FROM `kgbmxvry_u285514251_wms`.`stock_exchanges` WHERE `name` LIKE :exchange) AND `stock_exchanges`.`currency` = `exchange_rates`.`id`)),:daysrange,:name,:symbol,:volume,(SELECT id FROM `kgbmxvry_u285514251_wms`.`stock_exchanges` WHERE `name` LIKE :exchange)) ON DUPLICATE KEY UPDATE `averageDailyVolume` = :averagedailyvol,
				`change` = :change,`daysLow` = :dayslow,`daysHigh` = :dayshigh,`yearLow` = :yearlow,`yearHigh` = :yearhigh
				,`marketCapitalization` = :marketcap,`daysRange`= :daysrange,volume = :volume,`lastTradePriceOnly` = :lasttradeprice,`gbp_price` = :lasttradeprice*(1/(SELECT rate FROM `kgbmxvry_u285514251_wms`.`stock_exchanges`,`kgbmxvry_u285514251_wms`.`exchange_rates` WHERE `stock_exchanges`.`id` = 
(SELECT id FROM `kgbmxvry_u285514251_wms`.`stock_exchanges` WHERE `name` LIKE :exchange) AND `stock_exchanges`.`currency` = `exchange_rates`.`id`))";
